...
(function(global, undefined) {
  ...
  var GuessLanguage = function() {
      ...
      function identify(text, callback) {
        ...
        if (scripts["Hiragana"] + scripts["Katakana"] + scripts["Katakana Phonetic Extensions"] + scripts["CJK Japanese0"] + scripts["CJK Japanese1"] + scripts["CJK Japanese2"] + scripts["CJK Japanese3"]+ scripts["CJK Japanese4"]+ scripts["CJK Japanese5"]>= 0.2) {
          callback.apply(undefined, ["ja"]);
          return;
        }

        if (scripts["Bopomofo"] + scripts["Bopomofo Extended"] + scripts["KangXi Radicals"] + scripts["CJK Unified Ideographs"]>= 0.4) {
      ...
      function check(sample, langs, callback) {
        ...
        for (var i = 0, l = langs.length; i < l; i++) {
          ...
          var knownModel = models[lkey] || null;
      ...
      function createOrderedModel(content) {
        ...
        var contentArr = content.toLowerCase().split("");
        ...
        for (var j in trigrams) {
          sortedTrigrams[sortedTrigrams.length] = [j, trigrams[j]];
        }
